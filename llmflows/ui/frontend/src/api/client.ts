...
export const api = {
  ...
  archiveInboxBatch: (itemIds: string[]) => post<{ ok: boolean; archived: number }>("/api/inbox/archive-batch", { item_ids: itemIds }),
  approveImprovement: (itemId: string, selection?: string) =>
    post<{ ok: boolean; flow_id: string; version: number }>(
      `/api/inbox/${itemId}/improvement/approve`,
  ...
  buildRunnerImage: () => post<{ ok: boolean; building: boolean; message: string }>("/api/runner/build"),
  cancelRunnerBuild: () => post<{ ok: boolean; message: string }>("/api/runner/build/cancel"),
  getRunnerBuildLogs: (lines = 300) => get<{ lines: string[] }>(`/api/runner/logs?lines=${lines}`),
