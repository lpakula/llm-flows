...
// Matches lines like: "- link "News"" or "  - button "Submit" [disabled]"
...
async function takeSnapshot(): Promise<string> {
  ...
  try {
    ...
    // Track role+name occurrences so we can use .nth() for disambiguation
...
export default function activate(api: any) {
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { filename?: string }) {
      try {
        const p = await ensurePage();
        const fname = basename(params.filename || "screenshot.png");
        mkdirSync(ARTIFACTS_DIR, { recursive: true });
