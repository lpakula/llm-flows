# SUMMARIZE

Read all previous step results above as a whole. Write a single, concise markdown summary to `{{ run.dir }}/summary.md`. Do not summarize each step separately. Then stop.

## LANGUAGE

Write the entire summary in {{ summarizer_language }}.

## RULES

- Write in markdown format with headers, lists, and emphasis as appropriate
- Treat all steps as one coherent story, not a list of individual steps
- Focus on the substance and outcome, not the process
- Keep it concise but complete
- After writing the summary file, stop
