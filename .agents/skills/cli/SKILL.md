...
llmflows flow update my-flow --max-concurrent-runs 2
...
llmflows flow tools list my-flow

# Enable/disable a tool (available: web_search, browser)
llmflows flow tools add my-flow web_search
llmflows flow tools remove my-flow browser
```
...
```bash
llmflows flow var set my-flow API_KEY sk-abc123
llmflows flow var list my-flow
llmflows flow var remove my-flow API_KEY
```
